function activity2() {
    ...
    <div style='margin-top: 2vw;'>
        <br>
        <div class="fs-16px">
        <h5>Performing Gauss Seidel iterations</h5>
        <p>Learning Objective: To learn how to perform Gauss Seidel iterations</p>
        </div>
...
function start_act2() {
    ...
    let matrix_x1_inputs = get_matrix_html_with_title(3, 3, [
        ...
        [':', ':', ':'],
    ], "<div class='row'><div class='col-4'>X1</div><div class='col-4'>X2</div><div class='col-4'>X3</div></div>", 'inline-block', '60%');
    ...
    <div class='collapse divide' style='style='margin-top: 2vw; 'width: 80%; margin: auto;' id='tb2-box'>
        ...
        <h5>Enter the correct values upto 4 decimal places. Start with initial guess solution [0 0 0]T. Perform Gauss Seidel iterations</h5>
    ...
    setTimeout(() => show_step('tb2-box'), 300);
}
function ut_a_c() {
    let c = [];
    c.push(mat_c[0][0]);
...
function render_ut_a_c() {
    ...
    //4 decimal values in interation matrix
    for (let i = 0; i < mat_x1.length; i++) {
        for (let j = 0; j < mat_x1[0].length; j++) {
            mat_x1[i][j] = parseFloat(mat_x1[i][j].toFixed(4));
        }
    }
    div.innerHTML = `iteration matrix = ${get_matrix_html_with_title(10, 3, mat_x1, "<div class='row'><div class='col-4'>X1</div><div class='col-4'>X2</div><div class='col-4'>X3</div></div>", 'inline-block', '40%')}`;
}
//activity2();
